...
bool utils::IsOverlapping(const std::vector<Vector2f>& vertices, const Rectf& r)
{
	return IsOverlapping(vertices.data(), vertices.size(), r);
}

bool utils::IsOverlapping(const Vector2f* vertices, size_t nrVertices, const Rectf& r)
{
	// Verify whether one of vertices is in rect
	for (size_t i{ 0 }; i < nrVertices; ++i)
	{
		if (utils::IsPointInRect(vertices[i], r)) return true;
	}
	// No overlapping with edges, verify whether rectangle is completely inside the polygon

	if (IsPointInPolygon(Vector2f{ r.left, r.bottom }, vertices, nrVertices)) return true;
	if (IsPointInPolygon(Vector2f{ r.left, r.bottom + r.height }, vertices, nrVertices)) return true;
	if (IsPointInPolygon(Vector2f{ r.left + r.width, r.bottom + r.height }, vertices, nrVertices)) return true;
	if (IsPointInPolygon(Vector2f{ r.left + r.width, r.bottom }, vertices, nrVertices)) return true;
